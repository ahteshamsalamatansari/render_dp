...
async def attempt_route(origin, dest, today):
    """Run a single route attempt. Returns (success: bool, error_label: str|None).

    error_label is 'Timeout' for asyncio timeout, or the exception message otherwise.
    A fresh random session ID is generated inside scrape_route() on every call, so
    each retry automatically gets a new Bright Data IP."""
    try:
...
async def main():
    ...
    # ══════════════════════════════════════════════════════════════
    #  FIRST PASS: try every route once in declared order
    # ══════════════════════════════════════════════════════════════
    pending_retry = []   # list of (origin, dest, first_error)
    for i, (origin, dest) in enumerate(routes):
        print(f"\n[RUN] Starting scrape for {origin} -> {dest} (Max limit: 20 minutes)...")
        success, err = await attempt_route(origin, dest, today)

        if success:
            print(f"[PASS] Route {origin} -> {dest} completed on first attempt.")
        else:
            tag = "TIMEOUT" if err == "Timeout" else "FAIL"
            print(f"\n[{tag}] Route {origin} -> {dest} failed: {err}")
            print(f"    Queued for retry after first pass completes (cooldown will let session refresh).")
            pending_retry.append((origin, dest, err))
    ...
    # ══════════════════════════════════════════════════════════════
    #  SECOND PASS: retry failed routes after long cooldown
    # ══════════════════════════════════════════════════════════════
    final_failures = []
    if pending_retry:
        print(f"\n{'='*60}")
        print(f"  [RETRY] First pass complete. {len(pending_retry)} route(s) need retry:")
        for origin, dest, prev_err in pending_retry:
            print(f"    - {origin} -> {dest}  (was: {prev_err})")
        print(f"  Cooling down 60s to let Bright Data sessions refresh (new IPs on retry)...")
        print(f"{'='*60}")
        await asyncio.sleep(60)

        for i, (origin, dest, prev_err) in enumerate(pending_retry):
            print(f"\n[RETRY] Re-running {origin} -> {dest}  (previous error: {prev_err})")
            success, err = await attempt_route(origin, dest, today)

            if success:
                print(f"[RECOVERY] Route {origin} -> {dest} succeeded on retry.")
            else:
                tag = "TIMEOUT" if err == "Timeout" else "FAIL"
                print(f"[{tag}] Route {origin} -> {dest} still failed on retry: {err}")
                label = f"{origin}->{dest}"
                if err == "Timeout":
                    label += " (Timeout)"
                final_failures.append(label)

            if i < len(pending_retry) - 1:
                print("\n[WAIT] Waiting 30 seconds before next retry...")
    ...
    print(f"\n{'='*60}")
    if final_failures:
        print(f"  [DONE] Execution completed with failures (after retry).")
        print(f"  Failed Routes : {', '.join(final_failures)}")
        print(f"  Note: Other successful routes' CSV and Excel files are fully saved in output/ directory.")
    elif pending_retry:
        print(f"  [DONE] All selected routes executed successfully! ({len(pending_retry)} recovered on retry)")
        print(f"  Check output/ folder.")
    else:
        print(f"  [DONE] All selected routes executed successfully on first attempt! Check output/ folder.")
    print(f"{'='*60}\n")
