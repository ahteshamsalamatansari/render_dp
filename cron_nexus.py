...
Runs Nexus scraper, retries on connection errors, emails output on completion.
"""
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Nexus Airlines cron: scrape + email")
    parser.add_argument("--dry-run", action="store_true", help="Skip scraper, email existing files")
    args = parser.parse_args()

    log("=" * 55)
    log("🗓️  Nexus Airlines Scraper Cron")
