    ...
    async def run_route(self, origin_code, dest_code):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                initial_dt = dates[0]
                if not await self.select_departure_date(page, initial_dt):
                    self.write_unavailable_route_rows(
                        origin_code, dest_code,
                        "Could not select initial departure date",
                    )
                    return
...
if __name__ == "__main__":
    ...
    print(f"Today/system date: {today_dt().strftime('%A, %d-%m-%Y')}  (not scraped by default)")
    print(f"Timezone: {REX_TIMEZONE}")
    print(f"Locale: {REX_LOCALE}")
    print(f"Web Unlocker country: {BD_UNLOCKER_COUNTRY or 'default'}")
    print(f"Scrape starts: {start_dt.strftime('%A, %d-%m-%Y')}  (offset +{START_OFFSET_DAYS} day(s))")
